...
export function Price({titre,sousTitre, prix, infoPrix, l1, l2,l3,l4,buttonText,tag,addcss}: dataPrice){
    ...
    if(tag){
        ...
        buttonClass="bg-[#1E3A8A] text-white"
        fond="bg-[rgb(227,228,230)]"
        textcolor="text-[#505050]"
    }
    return(
        <div className={`bg-white rounded-3xl flex flex-col m-auto w-[400px] shadow-md ${withScale}`}>
            ...
            <Button className={`mb-8 mx-5 rounded-3xl p-7 ${buttonClass}`}>{buttonText}</Button>
        </div>
    )
}